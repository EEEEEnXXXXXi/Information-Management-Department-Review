...
> * IPO：為Data處理成Information的過程，當Data(資料)被收集以後，例如十年的消費者購物資料，經過Process處理以後，可以Output輸出為Information(資訊)作為後續的處理，比方說作為補貨的採購參考等。
...
### 記憶體與儲存
電腦中使用硬碟進行儲存，記憶體的部分有 **隨機存取記憶體RAM（Random Access Memory** 與 **唯讀記憶體ROM（Read Only Memory）** ，細節可以更分為可規化式唯讀記憶體（PROM）、可擦除可規劃式唯讀記憶體（EPROM）、靜態隨機存取記憶體（SRAM）、動態存取記憶體（DRAM）、快閃記憶體(Flash)、同步記憶體 (Concurrent Memory)等種類。
...
> Green computing(綠色運算)包含了減少電子的消耗，以及使用電腦時對於環境產生的汙染等，相關的策略包含了使用可以回收的材料、使用節電的硬體和相關的配備、調節管理的流程等。
**可參照綠色供應鏈的概念。**
